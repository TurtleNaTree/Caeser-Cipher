...
def caeser_cipher(text, shift)
    encrypted_text = text.split(//).map do |char|
        ...
        when (shift < 0)
            if char === char.capitalize
                char.match(/[A-Z]/) ? char.setbyte(0, char.getbyte(0) + shift < 65 ? char.getbyte(0) + shift + 26 : char.getbyte(0) + shift) : char
                char
            else 
                char.match(/[a-z]/) ? char.setbyte(0, char.getbyte(0) + shift < 97 ? char.getbyte(0) + shift + 26 : char.getbyte(0) + shift) : char
                char
            end
        end
    end
...
shift = shift.to_i
puts shift.to_i
puts "Text changed to #{caeser_cipher(input_string, shift)}"
